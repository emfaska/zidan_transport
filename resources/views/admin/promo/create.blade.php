...
<div class="container-fluid px-4 py-8 bg-[#f8faff]">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-black text-[#1a237e] tracking-tighter uppercase">Tambah Promo Baru</h1>
        <p class="text-[11px] font-bold text-gray-400 uppercase tracking-[0.3em] mt-1">Buat program diskon perjalanan yang menarik</p>
    </div>
    ...
    <form action="{{ route('admin.promo.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        @csrf
            ...
            <div class="bg-white rounded-[40px] shadow-2xl border border-gray-100 p-8 md:p-10">
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="judul" value="{{ old('judul') }}" placeholder="Contoh: Promo Ramadhan Berkah" class="w-full px-6 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:border-[#1a237e] focus:outline-none transition font-bold" required>
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="4" placeholder="Jelaskan detail promo di sini..." class="w-full px-6 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:border-[#1a237e] focus:outline-none transition font-bold">{{ old('deskripsi') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" name="potongan_persen" value="{{ old('potongan_persen') }}" placeholder="10" min="1" max="100" class="w-full px-6 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:border-[#1a237e] focus:outline-none transition font-bold" required>
                        ...
                        <div>
                            ...
                            <input type="text" name="kode_promo" value="{{ old('kode_promo') }}" placeholder="Contoh: ZIDANBERKAH" class="w-full px-6 py-4 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:border-[#1a237e] focus:outline-none transition font-bold uppercase">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="relative group cursor-pointer">
                    ...
                    <div id="image-preview-container" class="aspect-video bg-gray-50 border-2 border-dashed border-gray-200 rounded-[30px] flex flex-col items-center justify-center transition group-hover:bg-gray-100 overflow-hidden relative">
                        <div id="preview-placeholder" class="text-center">
                            ...
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-2">Upload Gambar</p>
                        </div>
