...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-6 border-b border-gray-100 flex flex-col lg:flex-row justify-between items-center gap-6">
        <div class="flex flex-col md:flex-row items-center gap-6 w-full lg:w-auto">
            <div>
                <h3 class="font-black text-[#1a237e] text-lg whitespace-nowrap">Daftar Armada</h3>
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Total: {{ $armadas->count() }} unit</p>
            </div>
            
            <!-- Search Bar -->
            <form action="{{ route('admin.armada.index') }}" method="GET" class="w-full md:w-80 relative group">
                ...
                <input type="text" name="search" value="{{ request('search') }}" 
                    placeholder="Cari nama, plat, atau jenis..." 
                ...
                >
                @if(request('search'))
                    <a href="{{ route('admin.armada.index') }}" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-red-500">
        ...
        <a href="{{ route('admin.armada.create') }}" class="w-full lg:w-auto px-6 py-4 bg-[#1a237e] text-white rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-blue-800 transition shadow-xl shadow-blue-900/20 active:scale-95">
