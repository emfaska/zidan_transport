...
@section('header_title', 'Edit Data Armada')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <form action="{{ route('admin.armada.update', $armada->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-8 border-b border-gray-100 bg-gray-50/50">
                <h3 class="font-black text-[#1a237e] text-lg mb-1 italic">Perbarui Data Armada</h3>
                <p class="text-xs text-gray-400 uppercase font-bold tracking-widest">ID Armada: #ARM-{{ str_pad($armada->id, 4, '0', STR_PAD_LEFT) }}</p>
            </div>
            ...
            <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="col-span-1">
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $armada->nama) }}" required placeholder="Contoh: Toyota HiAce Premium" 
                ...
                <div class="col-span-1">
                    ...
                    <select name="jenis" id="jenis" required 
                        ...
                        <option value="Minibus" {{ old('jenis', $armada->jenis) == 'Minibus' ? 'selected' : '' }}>Minibus (HiAce, ELF, etc)</option>
                        <option value="MPV" {{ old('jenis', $armada->jenis) == 'MPV' ? 'selected' : '' }}>MPV (Avanza, Xenia, etc)</option>
                        <option value="MPV Premium" {{ old('jenis', $armada->jenis) == 'MPV Premium' ? 'selected' : '' }}>MPV Premium (Innova, etc)</option>
                        <option value="SUV" {{ old('jenis', $armada->jenis) == 'SUV' ? 'selected' : '' }}>SUV</option>
                        <option value="Luxury" {{ old('jenis', $armada->jenis) == 'Luxury' ? 'selected' : '' }}>Luxury (Alphard, etc)</option>
                    </select>
                ...
                <div class="col-span-1">
                    ...
                    <select name="bbm" id="bbm" required 
                        ...
                        <option value="Pertalite" {{ old('bbm', $armada->bbm) == 'Pertalite' ? 'selected' : '' }}>Pertalite</option>
                        <option value="Pertamax" {{ old('bbm', $armada->bbm) == 'Pertamax' ? 'selected' : '' }}>Pertamax</option>
                        <option value="Solar" {{ old('bbm', $armada->bbm) == 'Solar' ? 'selected' : '' }}>Solar (Bio Solar/Dexlite)</option>
                        <option value="Dexlite" {{ old('bbm', $armada->bbm) == 'Dexlite' ? 'selected' : '' }}>Dexlite</option>
                        <option value="Listrik" {{ old('bbm', $armada->bbm) == 'Listrik' ? 'selected' : '' }}>Listrik (EV)</option>
                    </select>
                ...
                <div class="col-span-1">
                    ...
                    <input type="text" name="plat_nomor" id="plat_nomor" value="{{ old('plat_nomor', $armada->plat_nomor) }}" required placeholder="Contoh: AG 1234 XY" 
                ...
                <div class="col-span-1">
                    ...
                    <div class="relative">
                        <input type="number" name="kapasitas" id="kapasitas" value="{{ old('kapasitas', $armada->kapasitas) }}" required min="1" placeholder="16" 
                            class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#fbc02d] focus:border-[#fbc02d] outline-none transition text-sm font-semibold">
                ...
                <div class="col-span-1">
                    ...
                    <input type="number" name="tahun" id="tahun" value="{{ old('tahun', $armada->tahun) }}" min="1900" max="{{ date('Y')+1 }}" 
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#fbc02d] focus:border-[#fbc02d] outline-none transition text-sm font-semibold">
                ...
                <div class="col-span-1">
                    ...
                    <select name="status" id="status" required 
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#fbc02d] focus:border-[#fbc02d] outline-none transition text-sm font-semibold appearance-none">
                        <option value="tersedia" {{ old('status', $armada->status) == 'tersedia' ? 'selected' : '' }}>Tersedia (Siap Jalan)</option>
                        <option value="maintenance" {{ old('status', $armada->status) == 'maintenance' ? 'selected' : '' }}>Maintenance (Bengkel)</option>
                        <option value="terpakai" {{ old('status', $armada->status) == 'terpakai' ? 'selected' : '' }}>Terpakai (Dalam Tugas)</option>
                    </select>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="spesifikasi" id="spesifikasi" rows="3" placeholder="Contoh: AC Central, Audio System, USB Charger, Captain Seat, dll" 
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#fbc02d] focus:border-[#fbc02d] outline-none transition text-sm font-semibold resize-none">{{ old('spesifikasi', $armada->spesifikasi) }}</textarea>
                ...
                <div class="col-span-2">
                    ...
                    @if($armada->foto)
                        <div class="mb-4 w-48 h-32 rounded-xl overflow-hidden border border-gray-100 shadow-sm relative group">
                            <img src="{{ asset('storage/'.$armada->foto) }}" alt="{{ $armada->nama }}" class="w-full h-full object-cover">
                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition">
                                <span class="text-[9px] font-black text-white uppercase tracking-tighter">Foto Saat Ini</span>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="relative group w-full h-64 border-2 border-dashed border-gray-200 rounded-2xl overflow-hidden bg-gray-50 hover:bg-blue-50 transition hover:border-blue-200" id="drop-area">
                        ...
                        <div id="default-content" class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10 transition-opacity duration-300">
                            ...
                            <p class="text-xs font-bold text-gray-400 group-hover:text-blue-500">Klik atau seret gambar baru di sini</p>
                            <p class="text-[9px] text-gray-300 mt-1 uppercase tracking-tighter">Biarkan kosong jika tidak ingin mengubah foto</p>
                        </div>
                        ...
                        <div id="preview-container" class="absolute inset-0 hidden z-10 bg-white transition-opacity duration-300">
                            ...
                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition duration-300 pointer-events-none">
                                <span class="px-4 py-2 bg-white/20 backdrop-blur-sm rounded-lg text-white text-xs font-bold uppercase tracking-widest border border-white/30 truncate max-w-[90%]">
                                    <i class="bi bi-arrow-repeat mr-2"></i>Ganti Gambar Baru
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="p-8 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                <a href="{{ route('admin.armada.index') }}" class="px-6 py-3 border border-gray-200 text-gray-400 rounded-xl font-bold text-sm hover:bg-white hover:text-gray-600 transition flex items-center">Batal</a>
