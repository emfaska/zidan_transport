...
@section('title', 'Pesan Perjalanan - Zidan Transport')
...
@section('content')
    <!-- Progress Indicator (Mobile/Header) -->
    <div class="mb-8 flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
    ...
    <form action="{{ route('pelanggan.booking.store') }}" method="POST" id="mainBookingForm">
                        ...
                        <div class="flex gap-4 border-t border-indigo-100 md:border-t-0 md:border-l md:pl-8 pt-6 md:pt-0">
                           ...
                           <div class="space-y-3">
                               ...
                               <ul class="space-y-1.5">
                                   ...
                                   <li class="flex items-center gap-2 text-[11px] text-slate-600 font-medium">
                                       <i class="bi bi-x text-red-400 font-bold"></i> Penginapan Driver
                                   </li>
                               </ul>
                           </div>
                        </div>
                ...
                <div class="bg-white rounded-[40px] overflow-hidden shadow-2xl border border-slate-100 animate-up" style="animation-delay: 0.2s">
                    ...
                    <div class="px-8 -mt-8">
                        <div id="checkout-empty" class="bg-slate-50 rounded-[32px] p-10 border border-dashed border-slate-200 text-center flex flex-col items-center justify-center min-h-[220px]">
                            ...
                            <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">Pilih Armada Dahulu</p>
                        </div>
                        ...
                        <div id="price-box" class="hidden bg-white rounded-[32px] p-8 shadow-xl border border-slate-100 space-y-6">
                            ...
                            <div class="space-y-4">
                                <div class="flex gap-4">
                                    <div class="w-1.5 h-1.5 rounded-full bg-[#fbc02d] mt-1.5 shrink-0"></div>
                                ...
                                <div id="li-tol-included" class="hidden flex gap-4">
                                    ...
                                    <p class="text-[11px] font-black text-green-600 uppercase tracking-tight">Sudah Termasuk Tol</p>
                                </div>
                            </div>

                            <button type="button" onclick="showConfirmation()" class="w-full py-5 bg-[#fbc02d] text-[#1a237e] font-mont font-black uppercase tracking-widest text-[11px] rounded-2xl hover:scale-[1.02] shadow-lg transition-all">
                                Pesan Sekarang
                            </button>
        ...
        <div id="confirmationModal" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4">
            ...
            <div class="bg-white w-full max-w-lg rounded-[48px] overflow-hidden relative z-10 animate-up shadow-2xl">
                <div class="p-10 text-center bg-slate-50 border-b border-slate-100">
                    <div class="w-16 h-16 bg-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-5 text-white shadow-lg">
                    ...
                    <p class="text-slate-400 text-[9px] font-black uppercase tracking-[0.3em]">Cek rencana perjalanan Anda</p>
                </div>
                
                <div class="p-10 space-y-6">
                    <div class="grid grid-cols-2 gap-y-6 gap-x-8 text-left pb-6 border-b border-slate-100">
                        <div>
                            <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Tipe Trip</p>
                            <p id="modal-tipe" class="text-xs font-black text-[#1a237e] uppercase">-</p>
                        </div>
                        <div>
                            <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Armada</p>
                            <p id="modal-armada" class="text-xs font-bold text-slate-700 uppercase">-</p>
                        </div>
                        <div>
                    ...
                    <div class="space-y-4">
                        <div class="flex gap-4 p-4 rounded-2xl bg-blue-50/50">
                            <i class="bi bi-geo-alt-fill text-blue-600 mt-1"></i>
                            <div>
                                <p class="text-[8px] font-black text-slate-400 uppercase mb-1">Penjemputan</p>
                                <p id="modal-jemput" class="text-[11px] font-bold text-slate-800">-</p>
                            </div>
                        </div>
                        <div class="flex gap-4 p-4 rounded-2xl bg-slate-50">
                            <i class="bi bi-geo-fill text-slate-400 mt-1"></i>
                            <div>
                                <p class="text-[8px] font-black text-slate-400 uppercase mb-1">Tujuan</p>
                                <p id="modal-tujuan" class="text-[11px] font-bold text-slate-800">-</p>
                            </div>
                        </div>
                    </div>

                    <div class="flex flex-col gap-3 pt-6">
                        <button type="submit" class="w-full py-5 bg-[#1a237e] text-white font-mont font-black uppercase tracking-widest text-[11px] rounded-[24px]">
                            Konfirmasi & Pesan
                        </button>
                        <button type="button" onclick="closeConfirmation()" class="w-full py-4 text-slate-400 font-black uppercase tracking-widest text-[9px]">
                            Batal
                        </button>
                    </div>
                </div>
...
<script>
    ...
    document.addEventListener('turbo:load', function() {
        ...
        function refreshGrid() {
            ...
            baseRutes.forEach((r, i) => {
                ...
                // Status visual: grayscale & opacity jika sudah dibooking
                ...
                card.innerHTML = `
                    ${isBooked ? '<div class="absolute top-4 right-4 bg-red-100 text-red-600 px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest z-10 border border-red-200 shadow-sm">Tidak Tersedia</div>' : ''}
                    <div class="space-y-5 ${isBooked ? 'pointer-events-none' : ''}">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="font-bold text-[#1a237e] text-xs truncate uppercase">${r.armada ? r.armada.nama : 'Executive'}</h4>
                                <div class="flex items-center gap-1.5 mt-0.5">
                                    <i class="bi bi-people-fill text-[#fbc02d] text-[10px]"></i>
                                    <p class="text-[9px] text-slate-400 font-black uppercase">${r.armada ? r.armada.kapasitas + ' Seat' : 'Premium'}</p>
                                </div>
                            </div>
                        ...
                        <div class="price-display">
                            <p class="text-[10px] font-black text-slate-400 uppercase mb-1 current-price-label">Sekali Jalan</p>
                            <p class="text-xl font-mont font-black text-[#1a237e] current-price-value">${fmt(r.harga_paket)}</p>
                        </div>
                        ${hasPP ? `
                        <div class="pt-4 border-t border-slate-50 flex items-center justify-between">
                            <p class="text-[8px] font-black text-slate-300 uppercase">Opsi Trip</p>
                            <div class="flex bg-slate-100 p-1 rounded-xl gap-1">
                                <button type="button" class="type-btn active px-3 py-1.5 rounded-lg text-[8px] font-black uppercase" data-type="one_way">Sekali</button>
                                <button type="button" class="type-btn px-3 py-1.5 rounded-lg text-[8px] font-black uppercase" data-type="round_trip">PP</button>
                            </div>
                        </div>
                        ` : ''}
                    </div>
                ...
                card.addEventListener('click', (e) => {
                    if (isBooked) {
                        Swal.fire({ 
                            icon: 'error', 
                            title: 'Armada Penuh', 
                            text: 'Maaf, armada ini sudah dipesan oleh pelanggan lain pada tanggal tersebut.', 
                            background: '#fff', 
                            color: '#111', 
                            confirmButtonColor: '#1a237e',
                            customClass: {
                                popup: 'rounded-[32px]',
                                confirmButton: 'rounded-xl px-8 font-bold text-xs uppercase'
                            }
                        });
                        return;
                    }
                ...
                typeBtns.forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        ...
                        if (card.classList.contains('selected-package')) selectPackage(r, card, type);
                    });
        ...
        function updateFinalPrice() {
            ...
            } else { formInputs.liTol.classList.add('hidden'); }
    ...
    function showConfirmation() {
        if(!document.getElementById('rute_id_hidden').value) { 
            Swal.fire({ icon: 'warning', text: 'Pilih armada dahulu.', background: '#fff', color: '#111' }); return; 
        }
        const summaryData = {
            ...
            tipe: document.getElementById('tipe_perjalanan_hidden').value === 'round_trip' ? 'PP' : 'Sekali Jalan',
