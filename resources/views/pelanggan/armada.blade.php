...
@section('content')
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-500 font-medium">Temukan kendaraan terbaik untuk perjalanan Anda.</p>
    </div>
    ...
    <div class="mb-10 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div class="relative w-full md:w-96">
            ...
            <input type="text" id="armada-search" placeholder="Cari nama armada..." class="w-full pl-12 pr-6 py-4 bg-white border border-gray-100 rounded-2xl shadow-sm focus:ring-2 focus:ring-[#1a237e] focus:border-transparent transition-all outline-none font-bold text-sm">
        </div>
    ...
    <!-- Armada Grid -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12" id="armada-grid">
        @forelse($armadas as $armada)
        <div class="armada-card bg-white rounded-[32px] overflow-hidden shadow-sm border border-gray-100 hover:shadow-2xl hover:border-[#1a237e]/20 transition-all duration-500 group flex flex-col h-full">
            <!-- Armada Image -->
            <div class="relative h-48 overflow-hidden">
                ...
                <div class="absolute bottom-4 left-6 right-6 flex justify-between items-end">
                    <div>
                        <p class="text-[9px] font-black text-[#fbc02d] uppercase tracking-widest leading-none mb-1">Unit Armada</p>
                        <h3 class="text-lg font-black text-white uppercase tracking-tighter armada-name">{{ $armada->nama_armada }}</h3>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="bg-gray-50/50 p-3 rounded-xl border border-gray-100 flex items-center gap-3">
                        <div class="w-7 h-7 rounded-lg bg-yellow-100 flex items-center justify-center text-yellow-600">
                            <i class="bi bi-fuel-pump-fill text-xs"></i>
                        </div>
                        <span class="text-[10px] font-bold text-gray-700 uppercase tracking-tighter">{{ strtoupper($armada->bbm) }}</span>
                    </div>
                    @endif
                </div>
    ...
    <div class="bg-gradient-to-r from-[#1a237e] to-blue-800 rounded-[40px] p-10 text-white text-center shadow-2xl shadow-blue-900/20">
        <h2 class="text-3xl font-black mb-4 uppercase tracking-tighter">Masih Bingung?</h2>
...
<script>
    document.addEventListener('turbo:load', () => {
        const searchInput = document.getElementById('armada-search');
        ...
        searchInput.addEventListener('keyup', function() {
            ...
            cards.forEach(card => {
                const name = card.querySelector('.armada-name').innerText.toLowerCase();
